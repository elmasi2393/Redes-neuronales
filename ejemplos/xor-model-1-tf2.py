...
def v2_accuracy(y_true, y_pred):
    # Use tf.math instead of K.equal
    return tf.reduce_mean(tf.cast(tf.equal(y_true, tf.round(y_pred)), dtype=tf.float32), axis=-1)

# Model 
opti=tf.keras.optimizers.SGD(lr=0.01)

model.compile(optimizer=opti,
              loss='MSE', metrics=[v2_accuracy])
